...
func (h *Highlighter) storeRange(start int, end int, group Group, region *region, statesOnly bool) {
	if statesOnly {
		return
	}
	...
	for _, p := range patterns {
		if curRegion == nil || curRegion.group == curRegion.limitGroup || p.group == curRegion.limitGroup {
			...
			for _, m := range matches {
				h.storeRange(start+m[0], start+m[1], p.group, nil, statesOnly)
			}
		}
	}
}
			...
			if startMatches[startIdx][0] < lineLen {
				for endIdx := 0; endIdx < len(endMatches); endIdx++ {
					...
					if startMatches[startIdx][0] == endMatches[endIdx][0] {
						...
						h.storeRange(start+startMatches[startIdx][0], start+startMatches[startIdx][1], r.limitGroup, r, statesOnly)
						h.storeRange(start+startMatches[startIdx][1], start+endMatches[endIdx][0], r.group, r, statesOnly)
						h.storeRange(start+endMatches[endIdx][0], start+endMatches[endIdx][1], r.limitGroup, r, statesOnly)
						...
						h.lastStart = start
						h.lastEnd = start + endMatches[endIdx][1]
						h.storeRange(start, start+endMatches[endIdx][0], r.group, r, statesOnly)
						h.storeRange(start+endMatches[endIdx][0], start+endMatches[endIdx][1], r.limitGroup, r, statesOnly)
				...
				if nestedRegion || start+startMatches[startIdx][0] < h.lastStart || h.lastEnd < start+startMatches[startIdx][0] {
					...
					h.storeRange(start+startMatches[startIdx][0], start+startMatches[startIdx][1], r.limitGroup, r, statesOnly)
					h.storeRange(start+startMatches[startIdx][1], start+lineLen, r.group, r, statesOnly)
		...
		if curRegion == r {
			if (len(startMatches) == 0 && len(endMatches) > 0) || (samePattern && (len(startMatches) == len(endMatches))) {
				for _, endMatch := range endMatches {
					...
					h.storeRange(start, start+endMatch[0], r.group, r, statesOnly)
					h.storeRange(start+endMatch[0], start+endMatch[1], r.limitGroup, r, statesOnly)
				...
				h.storeRange(start, start+lineLen, curRegion.group, r, statesOnly)
			}
		}
	}
