...
func (h *Highlighter) highlightRegions(fullHighlights []Group, start int, lineNum int, line []byte, curRegion *region, regions []*region, nestedRegion bool, statesOnly bool) {
			...
			if startMatches[startIdx][0] < lineLen {
				for endIdx := 0; endIdx < len(endMatches); endIdx++ {
					...
					if startMatches[startIdx][0] == endMatches[endIdx][0] {
						...
						// log.Println("start < end")
						update := false
						if h.lastStart == 0 || h.lastStart < start+endMatches[endIdx][1] {
							h.lastStart = start + startMatches[startIdx][0]
							h.lastEnd = start + endMatches[endIdx][1]
							update = true
						}
						h.highlightRange(fullHighlights, start+startMatches[startIdx][0], start+startMatches[startIdx][1], r.limitGroup, statesOnly)
						h.highlightRange(fullHighlights, start+startMatches[startIdx][1], start+endMatches[endIdx][0], r.group, statesOnly)
						h.highlightRange(fullHighlights, start+endMatches[endIdx][0], start+endMatches[endIdx][1], r.limitGroup, statesOnly)
						...
						h.lastStart = start
						h.lastEnd = start + endMatches[endIdx][1]
						h.highlightRange(fullHighlights, start, start+endMatches[endIdx][0], r.group, statesOnly)
						h.highlightRange(fullHighlights, start+endMatches[endIdx][0], start+endMatches[endIdx][1], r.limitGroup, statesOnly)
				...
				if nestedRegion || start+startMatches[startIdx][0] <= h.lastStart || h.lastEnd <= start+startMatches[startIdx][0] {
					...
					h.highlightRange(fullHighlights, start+startMatches[startIdx][0], start+startMatches[startIdx][1], r.limitGroup, statesOnly)
					h.highlightRange(fullHighlights, start+startMatches[startIdx][1], start+lineLen, r.group, statesOnly)
		...
		if curRegion == r {
			if (len(startMatches) == 0 && len(endMatches) > 0) || (samePattern && (len(startMatches) == len(endMatches))) {
				for _, endMatch := range endMatches {
					...
					h.highlightRange(fullHighlights, start, start+endMatch[0], r.group, statesOnly)
					h.highlightRange(fullHighlights, start+endMatch[0], start+endMatch[1], r.limitGroup, statesOnly)
					...
					curRegion = h.lastRegion
					h.highlightRegions(fullHighlights, start+endMatch[1], lineNum, util.SliceEnd(line, endMatch[1]), curRegion, h.Def.rules.regions, false, statesOnly)
					break
				}
			} else if len(startMatches) == 0 && len(endMatches) == 0 {
				// no start and end found in this region
				h.highlightRange(fullHighlights, start, start+lineLen, curRegion.group, statesOnly)
			}
		}
	}
	...
	if curRegion != nil && !nestedRegion {
		...
		if curRegion == h.lastRegion && curRegion.parent != nil {
			...
			h.highlightRegions(fullHighlights, start, lineNum, line, curRegion, regions, true, statesOnly)
		}
	}
}
