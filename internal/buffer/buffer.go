...
// If bufferCommand.cursorLoc is {-1, -1} the location does not overwrite what the cursor location
// would otherwise be (start of file, or saved cursor position if `savecursor` is
// enabled)
func NewBufferFromFileWithCommand(path string, btype BufType, bufferCommand BufferCommand) (*Buffer, error) {
	var err error
	filename := path
...
func NewBufferFromFile(path string, btype BufType) (*Buffer, error) {
	return NewBufferFromFileWithCommand(path, btype, emptyBufferCommand)
}

// NewBufferFromStringWithCommand creates a new buffer containing the given string
// with a cursor loc and a search text
func NewBufferFromStringWithCommand(text, path string, btype BufType, bufferCommand BufferCommand) *Buffer {
...
func NewBuffer(r io.Reader, size int64, path string, btype BufType, bcommand BufferCommand) *Buffer {
	...
	case None, StartCursorSearchText:
		// We can just search from current cursor and move it accordingly
		// If the search text is empty FindNext won't do anything
