	...
	go func() {
		...
		for {
			...
			case sr := <-saveRequestChan:
				size, err := sr.buf.safeWrite(sr.path, sr.withSudo)
...
func (b *Buffer) saveToFile(filename string, withSudo bool, autoSave bool) error {
	...
	saveRequestChan <- saveRequest{b, absFilename, withSudo, saveResponseChan}
...
// backup file first.
func (b *Buffer) safeWrite(path string, withSudo bool) (int, error) {
