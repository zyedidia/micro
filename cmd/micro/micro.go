...
func LoadInput(args []string) []*buffer.Buffer {
	...
	var order = buffer.None

	if posIndex != -1 && searchIndex != -1 {
		if posIndex < searchIndex {
			order = buffer.StartCursorSearchText
		} else {
			order = buffer.SearchTextStartCursor
		}
	}

	var command = buffer.BufferCommand{
